...
class _EntryPointState extends State<EntryPoint> {
  ...
  Future<void> loadUserId() async {
    try {
      ...
      // Auto-sync when app starts if user is logged in
      if (uid != null && uid.isNotEmpty) {
        // Sync will happen automatically when AuthProvider is initialized
        log("[EntryPoint] User found, sync will be triggered automatically");
      }
    } catch (e, s) {
      log("[EntryPoint] Failed to load SharedPreferences: $e", stackTrace: s);
      if (!mounted) return;
